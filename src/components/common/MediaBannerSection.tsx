...
interface MediaBannerSectionProps {
    ...
    promoCard?: { // Optional promo card details
        imageUrl: string;
        imageAlt: string;
        category: string;
        title: string;
                ...
                {/* Main Text & CTA (Bottom Left) */}
                 <div className="flex flex-col justify-end items-start flex-grow pb-10 lg:pb-16 mt-auto">
                    <div className="space-y-3 lg:space-y-4">
                        {category && (
                            <p className="text-secondary-medium-sm lg:text-secondary-medium-base opacity-90">
                                {category}
                            </p>
                        )}
                        <h3 className="text-primary-medium-3xl lg:text-primary-medium-4xl">
                           {/* Basic title, wrap words for animation if needed later */}
                           {title}
                         </h3>
                         <Link href={ctaLink} passHref>
                           <button type="button" className="primary-white-button"> {/* Use button styles */}
                                {ctaText}
                           </button>
                         </Link>
                    </div>
